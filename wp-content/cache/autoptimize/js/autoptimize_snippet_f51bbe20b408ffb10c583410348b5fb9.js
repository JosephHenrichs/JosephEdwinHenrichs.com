try{;(function($){$.fn.jetStickySection=function(options){var defaults={topSpacing:0,zIndex:'',stopper:$('.sticky-stopper'),stickyClass:false};var settings=$.extend({},defaults,options);function checkIndex(){if(typeof settings.zIndex=='number'){return true;}else{return false;}}
...
if($window.innerHeight()>thisHeight){$this.on('jetStickySection:activated',stickyScroll);$window.on('scroll',stickyScroll);$window.on('touchmove',stickyScroll);$window.on('resize',stickyScroll);$this.on('jetStickySection:detach',detachStickyScroll);}});};})(jQuery);}catch(e){}